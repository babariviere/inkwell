...
use llvm_sys::core::{
    ...
    LLVMGetBasicBlocks, LLVMGetElementType, LLVMGetEntryBasicBlock, LLVMGetFirstBasicBlock,
    LLVMGetFirstParam, LLVMGetFunctionCallConv, LLVMGetGC, LLVMGetIntrinsicID,
    LLVMGetLastBasicBlock, LLVMGetLastParam, LLVMGetLinkage, LLVMGetNextFunction, LLVMGetNextParam,
    LLVMGetParam, LLVMGetParams, LLVMGetPreviousFunction, LLVMGetReturnType, LLVMIsAFunction,
    LLVMIsConstant, LLVMSetFunctionCallConv, LLVMSetGC, LLVMSetLinkage, LLVMSetParamAlignment,
    LLVMTypeOf,
};
...
use values::{BasicValueEnum, GlobalValue, MetadataValue, Value};
...
impl FunctionValue {
    ...
    pub fn get_entry_basic_block(&self) -> Option<BasicBlock> {
        let bb = unsafe { LLVMGetEntryBasicBlock(self.as_value_ref()) };
    ...
    pub fn get_return_type(&self) -> BasicTypeEnum {
        let type_ =
            unsafe { LLVMGetReturnType(LLVMGetElementType(LLVMTypeOf(self.fn_value.value))) };
    ...
    pub fn add_attribute(&self, index: u32, attribute: Attribute) {
        unsafe { LLVMAddAttributeAtIndex(self.as_value_ref(), index, attribute.attribute) }
    }
    ...
    pub fn count_attributes(&self, index: u32) -> u32 {
        unsafe { LLVMGetAttributeCountAtIndex(self.as_value_ref(), index) }
    }
    ...
    pub fn remove_enum_attribute(&self, index: u32, kind_id: u32) {
        unsafe { LLVMRemoveEnumAttributeAtIndex(self.as_value_ref(), index, kind_id) }
    }
    ...
    pub fn get_enum_attribute(&self, index: u32, kind_id: u32) -> Option<Attribute> {
        let ptr = unsafe { LLVMGetEnumAttributeAtIndex(self.as_value_ref(), index, kind_id) };
...
impl NamedValue for FunctionValue {
