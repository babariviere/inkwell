...
pub use values::enums::{AggregateValueEnum, AnyValueEnum, BasicMetadataValueEnum, BasicValueEnum};
...
pub use values::instruction_value::{InstructionOpcode, InstructionValue};
...
pub use values::traits::{
    AggregateValue, AnyValue, BasicValue, FloatMathValue, IntMathValue, NamedValue,
    PointerMathValue,
};
pub use values::vec_value::VectorValue;

use llvm_sys::core::{
    LLVMDumpValue, LLVMGetMetadata, LLVMHasMetadata, LLVMIsAInstruction, LLVMIsConstant,
    LLVMIsNull, LLVMIsUndef, LLVMPrintTypeToString, LLVMPrintValueToString, LLVMReplaceAllUsesWith,
    LLVMSetMetadata, LLVMTypeOf,
};
...
impl Value {
    ...
    fn set_name(&self, name: &str) {
        ...
        )))]
        {
            ...
            unsafe { LLVMSetValueName2(self.value, name.as_ptr() as *const i8, name.len()) }
        }
    }
    ...
    fn print_to_string(&self) -> LLVMString {
        let c_string = unsafe { LLVMPrintValueToString(self.value) };
    ...
    fn has_metadata(&self) -> bool {
        unsafe { LLVMHasMetadata(self.value) == 1 }
    }
    ...
    fn get_metadata(&self, kind_id: u32) -> Option<MetadataValue> {
        let metadata_value = unsafe { LLVMGetMetadata(self.value, kind_id) };
    ...
    fn set_metadata(&self, metadata: MetadataValue, kind_id: u32) {
        unsafe { LLVMSetMetadata(self.value, kind_id, metadata.as_value_ref()) }
    }
